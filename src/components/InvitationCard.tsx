...
import { Calendar, Clock, MapPin, Image as ImageIcon, ExternalLink, Download, Loader2, ChevronsDown, CalendarPlus } from 'lucide-react';
...
export const InvitationCard: React.FC<InvitationCardProps> = ({ guestName, onOpenGallery, onOpenMap, showDetails = true }) => {
  ...
  useEffect(() => {
    // Target Date: Feb 15, 2026 12:30:00
  ...
  const addToCalendarUrl = () => {
    ...
    // Time: Feb 15, 2026 12:30 PM IST -> 07:00 AM UTC
    const startDate = "20260215T070000Z"; 
    // End Time: Approx 3 hours later -> 10:00 AM UTC
  ...
  const containerVariants: Variants = {
    ...
    visible: {
      opacity: 1,
      y: 0,
      transition: { duration: 0.5, staggerChildren: 0.2, delayChildren: 2.2 }
    }
  };

  const itemVariants: Variants = {
    hidden: { opacity: 0, y: 10, x: -5 },
    visible: { opacity: 1, y: 0, x: 0, transition: { type: "spring", stiffness: 100, damping: 12 } }
  };
  ...
  const babyTitleVariants: Variants = {
    ...
    visible: { 
      ...
      transition: { delay: 0.5, duration: 1.0, type: "spring", bounce: 0.5 }
    }
  };
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 flex flex-col h-full w-full max-w-[90%] mx-auto">
            ...
            <motion.div className="space-y-1 mt-2" initial="hidden" animate={showDetails ? "visible" : "hidden"}>
                ...
                <motion.div variants={guestNameVariants} className="relative py-1 perspective-[500px] z-20">
                    <h2 className="text-2xl md:text-3xl font-['Great_Vibes'] text-amber-600 leading-tight"
                        style={{ textShadow: `0 1px 0 #d97706, 0 2px 0 #b45309` }}>
                        {guestName}
                    </h2>
                </motion.div>
            ...
            <div className="py-2 md:py-4 flex flex-col items-center">
                ...
                <motion.div className="relative perspective-[500px] py-2" variants={babyTitleVariants} initial="hidden" animate={showDetails ? "visible" : "hidden"}>
                    <h1 className="text-4xl md:text-6xl font-['Cinzel'] font-black text-amber-500 leading-none tracking-wide"
            ...
            <motion.div className="bg-amber-50/50 p-3 md:p-4 rounded-xl border border-amber-100 shadow-sm space-y-2 mb-2"
                ...
                <motion.div variants={itemVariants} className="flex items-center gap-3 text-left cursor-pointer hover:bg-amber-100/50 p-1.5 -mx-1.5 rounded-lg transition-colors group relative"
                    ...
                    <div className="flex-1">
                        <div className="flex justify-between items-center">
                            <p className="text-[10px] text-gray-500 uppercase font-bold group-hover:text-amber-700">Venue</p>
                            <ExternalLink size={10} className="text-amber-400 opacity-0 group-hover:opacity-100" />
                        </div>
                        <p className="text-xs md:text-sm font-semibold text-slate-700 leading-tight">SukhSundar Bhavan, Plot No. 7, Vrundavan Colony, Nagpur</p>
                    </div>
                </motion.div>
            </motion.div>

            <motion.div className="mt-auto flex gap-2 justify-center pb-2 relative z-50"
                ...
                <button onClick={handleDownload} disabled={isDownloading}
                    ...
                    {isDownloading ? <><Loader2 size={12} className="animate-spin" /> Saving</> : <><Download size={12} /> Download</>}
                </button>
            </motion.div>
