...
export const NameSuggestionModal: React.FC<NameSuggestionModalProps> = ({ onClose, onSuccess }) => {
  ...
  const handleVote = (id: string) => {
    const updated = suggestionService.voteForName(id);
    setSuggestions(updated);
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    addName(newName, meaning);
  };

  const addName = (nameToAdd: string, meaningToAdd: string) => {
    if (nameToAdd.trim()) {
      suggestionService.addSuggestion(nameToAdd, meaningToAdd);
      setSuggestions(suggestionService.getSuggestions());
      setNewName('');
      setMeaning('');
      setActiveTab('vote'); 
